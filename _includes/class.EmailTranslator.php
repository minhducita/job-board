<?php
	class EmailTranslator{
		...
		public function GetPublishToAdminMsg($data)
		{
			...
			$msg = str_replace('{JOB_DELETE_URL}', $data['delete_url'], $msg); //add
		...
		public function GetPublishToUserMsg($data)
		{
			...
			if(isset($data['ACTIVE_EMAIL_MESSAGE']))
				$msg = str_replace('{ACTIVE_EMAIL_MESSAGE}', $data['ACTIVE_EMAIL_MESSAGE'], $msg);
			else
				$msg = str_replace('{ACTIVE_EMAIL_MESSAGE}', ' ', $msg);
			$msg = str_replace('{JOB_URL}', $data['job_url'], $msg);
			$msg = str_replace('{JOB_EDIT_URL}', $data['edit_url'], $msg);
			//$msg = str_replace('{JOB_DEACTIVATE_URL}', $data['deactivate_url'], $msg);
			$msg = str_replace('{JOB_DELETE_URL}', $data['delete_url'], $msg); //add
			$msg = str_replace('{JOB_ID}', $data['id'], $msg); //add
			$msg = str_replace('{SITE_NAME}',SITE_NAME, $msg);
			$msg = self::ToNl($msg);
			return $msg;
		}
		...
		/*Nam cap nhat*/
		public function GetPostDateEndSubject()
		{
			$msg = (string)$this->xml->PostDateEnd->subject;
			return $msg;
		}
		public function GetPostDateEndMsg($data){
			$msg = (string)$this->xml->PostDateEnd->message;
			$msg = str_replace('{ID}', $data['id'], $msg);
			$msg = str_replace('{LINK_PAGE}', $data['link_page'], $msg);
			$msg = str_replace('{LINK_EDIT}', $data['link_edit'], $msg);
			$msg = str_replace('{LINK_DELETE}',$data['link_delete'], $msg);
			$msg = self::ToNl($msg);
			return $msg;
		}
		/*Nam cap nhat*/
