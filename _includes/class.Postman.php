...
class Postman
{
	...
	public function MailApplyOnline($data)
	{
		...
		//$mailer->AddBCC($address_admin, $name_admin = '');
		$mailer->AddBCC(NOTIFY_EMAIL, SITE_NAME);
    	$mailer->Subject = $subject;
    	$mailer->Body = $this->nl2br($msg);
    	$mailer->AltBody = $msg;
	...
	public function MailPublishToAdmin($data)
	{
		...
		$data['delete_url'] =  BASE_URL . "delete/" . $data['id'] . "/" . $data['auth'] . "/"; //add
...
//		$mailer->Send();
	...
	public function MailPublishPendingToUser($poster_email)
	{
		...
		$poster_email = trim($poster_email);
		if ($poster_email != '' && validate_email($poster_email))
		{
			...
			$message = Swift_Message::newInstance()
				->setSubject($subject)
				->setFrom(array(NOTIFY_EMAIL => SITE_NAME))
				->setTo($poster_email)
				->setBody($msg);
	...
	{	
		...
		$data['delete_url'] =  $url . "delete/" . $data['id'] . "/" . $data['auth'] . "/"; //add
     ...
     */
	public function MailPostDateEnd($datas){
		$list_id = "";
		require_once 'swiftmailer/swift_required.php';

		if($datas) foreach($datas as $data){
			$subject = $this->emailTranslator->GetPostDateEndSubject();
			$msg = '';

			$data['link_page'] = BASE_URL . URL_JOB .'/' . $data['id'] . '/' . $data['url_title'] . '/';

			$data['link_edit'] = BASE_URL . "post/" . $data['id'] . "/" . $data['auth'] . "/";

			//$data['deactivate_url'] =  BASE_URL . "deactivate/" . $data['id'] . "/" . $data['auth'] . "/";
			$data['link_delete'] =  BASE_URL . "delete/" . $data['id'] . "/" . $data['auth'] . "/"; //add

			$data['poster_ip'] = $_SERVER['REMOTE_ADDR'];

			$msg = $this->emailTranslator->GetPostDateEndMsg($data);
