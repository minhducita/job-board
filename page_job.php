...
if ($job->Exists() && $job->GetTempStatus() == 0 && $job->GetActiveStatus() == 1) {
    ...
    $info["root_image1"] = str_replace("uploads/job","uploads/job/root",$info["image1"]);
    $info["root_image2"] = str_replace("uploads/job","uploads/job/root",$info["image2"]);
    $info["root_image3"] = str_replace("uploads/job","uploads/job/root",$info["image3"]);

    $info['nationality'] = $nationalities[$info['nationality']];
    ...
    $active_arrow = 0;
    if($info['image1'] && $info['image1'] != 'NULL') $active_arrow++;
    if($info['image2'] && $info['image2'] != 'NULL') $active_arrow++;
    if($info['image3'] && $info['image3'] != 'NULL') $active_arrow++;
    $smarty->assign('active_arrow',$active_arrow);


    $template = !$isMobile ? 'job.tpl': 'MobileTemplates/views/jobs/detail.tpl';
} else {
