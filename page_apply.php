...
if($_FILES['apply_cv'] && $_FILES['apply_cv']['size'] < MAX_CV_SIZE){
	...
	if (move_uploaded_file($_FILES['apply_cv']['tmp_name'], APP_PATH . FILE_UPLOAD_DIR . $filename))
	{
		$attachment = $filename;
		$_SESSION['attachment']['name'] = $_FILES['apply_cv']['name'];
		$_SESSION['attachment']['link'] = APP_PATH . FILE_UPLOAD_DIR . $filename;
	}
	else
	{
		$attachment = '';
...
if (empty($errors))
{
	...
	$data = array('apply_email' => $apply_email,
		...
		'job_title' => $j->mTitle,
		'attachment_path' => $attachment ? APP_PATH . FILE_UPLOAD_DIR . $attachment : $_SESSION['attachment']['link'],
		'attachment_filename' => $attachment ? $attachment: $_SESSION['attachment']['name'],
		'job_id' => $job_id);
